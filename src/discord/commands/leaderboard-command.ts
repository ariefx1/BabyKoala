...
export default class LeaderboardCommand implements BaseCommand {
  ...
  public async execute(interaction: CommandInteraction): Promise<void> {
    try {
      ...
      const { seasonStartDate, committeeIds } = await querySettings();
      const ephemeral: boolean = !committeeIds.includes(interaction.user.id);
      ...
      await interaction.editReply(`Query took ${t1 - t0} milliseconds`);
